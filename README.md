...
|CEP_ADMIN_HTTP_API_KEYS|When set it enables admin http api security via ```Authorization: apiKey [APIKEY]```. The value will allow to set 1 or more API keys separated by blank space character. Multiple API keys provides an easy way to establish a policy of API keys rotation.|string||
