    ...
    describe('rules', () => {
        ...
        describe('get', () => {
            ...
            it('should return 200 with array of rules filtered by search query string', async () => {
                const eventType = await createEventType(adminServer);
                const target = await createTarget(adminServer);
                await createRule(adminServer, eventType, target, 'my ru?e');
                await createRule(adminServer, eventType, target, 'my rute');
                await createRule(adminServer, eventType, target, 'my ru?e2');
                await createRule(adminServer, eventType, target, 'my ruue');
                await createRule(adminServer, eventType, target, 'my ru?e1');
        ...
        describe('post', () => {
            ...
            it('should return 409 when try to create a rule with the same name', async () => {
                const eventType = await createEventType(adminServer);
                const target = await createTarget(adminServer);
                const responseCreaterule = await adminServer.inject({
                    method: 'POST',
                    url: '/rules',
                    body: {
                        name: 'same name',
                ...
                const responseCreaterule2 = await adminServer.inject({
                    method: 'POST',
                    url: '/rules',
                    body: {
                        name: 'same name',
        ...
        describe('delete', () => {
            ...
            it('should return 204 when rule exists', async () => {
                const eventType = await createEventType(adminServer);
                const target = await createTarget(adminServer);
                const createdRule = await createRule(adminServer, eventType, target);
        ...
        async function createRule(adminServer, eventType, target, name = 'a rule') {
            const createResponse = await adminServer.inject({
                method: 'POST',
                url: '/rules',
                body: {
